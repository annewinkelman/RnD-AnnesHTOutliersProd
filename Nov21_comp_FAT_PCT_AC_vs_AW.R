# AC data
# In the directory
# awk -F, '$15 > 0 && $15 < 9{ print FILENAME "," $0 }' *out > Nov21_FAT_PCT_outliers_AC.csv
...
ac_data <- read.csv("Nov21_FAT_PCT_outliers_AC.csv", header=F)
ncol(ac_data)
...
aw_data <- read.csv("Nov21_FAT_PCT_allkeysqt4rpass12.csv")
...
if(sum(!ac_cat) > 0){
   write.csv(ac_data_noaw, "Nov21_comp_FAT_PCT_AC_noAW.csv", row.names=F)
   }
...
if(sum(!aw_cat) > 0){
   write.csv(aw_data_noac, "Nov21_comp_FAT_PCT_AW_noAC.csv", row.names=F)
   }
...
write.table(ac_aw_counts,"Nov21_comp_FAT_PCT_AC_vs_AW_counts.txt")
