...
# files
two_pass_data <- read.csv("Nov21_FAT_PCT_twopassoutsum.csv")

two_pass_data[1:4,]

# pass1 keys
# vars: "file","anmlkey","Contemporary.Group","FAT_PCT","hilo","ndata"
keys_pass1 <- read.csv("Nov21_FAT_PCT_allkeysqt4rpass1.csv")
keys_pass1[1:4,]
...
# vars: "file","anmlkey","Contemporary.Group","FAT_PCT","hilo","ndata"
if(file.exists("Nov21_FAT_PCT_allkeysqt4rpass2.csv")){
keys_pass2 <- read.csv("Nov21_FAT_PCT_allkeysqt4rpass2.csv")
keys_pass2[1:4,]
keys_pass2$pass <- 2
}
...
pdf("Nov21_FAT_PCT_outliers.pdf")
...
#for (ifile in 1:1) {
     ...
     for (icgrp in 1:length(work_data_OUT)){
	  ...
	  if(sum(work_file_cat) > 0){
	     # still need to remove records below the cut-off value
             sel_cat <- work_data_cgrp$FAT_PCT > 0.5 & work_data_cgrp$abnormal == 0
	     ...
	     work_m1 <- merge(work_data_cgrp, work_sel_keys[,c("anmlkey","Contemporary.Group","FAT_PCT","hilo","pass")], all.x=T, all.y=T)
	     ...
	     # ggplot     
	     work_plot <- ggplot(work_m1, aes(x=FAT_PCT,fill=factor(pass))) +
