...
ac_data <- read.csv("Jul21_AM_VOL_outliers_AC.csv", header=F)
ncol(ac_data)
...
aw_data <- read.csv("Jul21_AM_VOL_allkeysqt4rpass12.csv")
...
if(sum(!ac_cat) > 0){
   write.csv(ac_data_noaw, "Jul21_comp_AM_VOL_AC_noAW.csv", row.names=F)
   }
...
if(sum(!aw_cat) > 0){
   write.csv(aw_data_noac, "Jul21_comp_AM_VOL_AW_noAC.csv", row.names=F)
   }
...
write.table(ac_aw_counts,"Jul21_comp_AM_VOL_AC_vs_AW_counts.txt")
