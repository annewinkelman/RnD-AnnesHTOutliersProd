...
# vars: "AGE_GRP_CD","SSN_CD","CARRY","SAMPLE_REGIME_CD","anmlkey","abnormal","PROT_PCT","PROT_PCT","PROT_PCT","PROT_PCT","TAG","Contemporary.Group"
...
# files
two_pass_data <- read.csv("Jun21_PROT_PCT_twopassoutsum.csv")

two_pass_data[1:4,]

# pass1 keys
# vars: "file","anmlkey","Contemporary.Group","PROT_PCT","hilo","ndata"
keys_pass1 <- read.csv("Jun21_PROT_PCT_allkeysqt4rpass1.csv")
keys_pass1[1:4,]
...
# vars: "file","anmlkey","Contemporary.Group","PROT_PCT","hilo","ndata"
if(file.exists("Jun21_PROT_PCT_allkeysqt4rpass2.csv")){
keys_pass2 <- read.csv("Jun21_PROT_PCT_allkeysqt4rpass2.csv")
keys_pass2[1:4,]
keys_pass2$pass <- 2
}
...
pdf("Jun21_PROT_PCT_outliers.pdf")
...
#for (ifile in 1:1) {
     ...
     for (icgrp in 1:length(work_data_OUT)){
	  ...
	  if(sum(work_file_cat) > 0){
	     ...
	     work_m1 <- merge(work_data_cgrp, work_sel_keys[,c("anmlkey","Contemporary.Group","PROT_PCT","hilo","pass")], all.x=T, all.y=T)
	     ...
	     # ggplot     
	     work_plot <- ggplot(work_m1, aes(x=PROT_PCT,fill=factor(pass))) +
