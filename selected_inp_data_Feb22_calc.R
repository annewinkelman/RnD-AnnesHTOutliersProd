# ls -1v Feb22*status1.txt > list_Feb22_status1_files

#status1_file_list <- read.table("list_jul21_status1_files")
...
# can also do:
# ls -1v Feb22*status1.txt > list_Feb22_status1_files
# ls -1v Feb22*stats*csv > list_Feb22_outlier_stats_files
# awk -F"-" '{print $2}' list_Feb22_status1_files > list_Feb22_status1_files_names
# awk -F"-" 'NR==FNR{a[$1]; next} $2 in a' list_Feb22_status1_files_names list_Feb22_outlier_stats_files > selected_Feb22_outlier_stats_files

sel_file_names_list <- read.table("list_Feb22_status1_files_names")
...
sel_stats_file_list <- read.table("selected_Feb22_outlier_stats_files")

sel_stats_file_list[1:5,]
nrow(sel_stats_file_list)

# ls -1v Feb22*cnts*csv > list_Feb22_outlier_cnts_files
# awk -F"-" 'NR==FNR{a[$1]; next} $2 in a' list_Feb22_status1_files_names list_Feb22_outlier_cnts_files > selected_Feb22_outlier_cnts_files

sel_cnts_file_list <- read.table("selected_Feb22_outlier_cnts_files")
sel_cnts_file_list[1:5,]
nrow(sel_cnts_file_list)


# ls -1v Feb22/*inp > all_Feb22_inp_files
# awk -F'[/.]' 'NR==FNR{a[$1]; next} $2 in a' list_Feb22_status1_files_names all_Feb22_inp_files > selected_Feb22_input_files

sel_data_file_list <- read.table("selected_Feb22_input_files")
...
#      print("work_trait_all_sel",work_trait_all_sel)
	...
	if(!work_file_size){
	   work_trait_name <- trait_list[itrait]
       	   work_dir <- paste0("Feb22_sel_",work_trait_name,"/")
       	   # cnts
