# input data:
# ls -1v Jun21_sel_AM_VOL/*inp > list_Jun21_sel_AM_VOL_inp_files
# ls -1v Jun21_sel_AM_VOL/*stats > list_Jun21_sel_AM_VOL_stats_files

work_ls_num1_run <- paste0("ls -1v Jun21_sel_AM_VOL/*inp > list_Jun21_sel_AM_VOL_inp_files")
work_ls_num2_run <- paste0("ls -1v Jun21_sel_AM_VOL/*stats > list_Jun21_sel_AM_VOL_stats_files")
...
flist_data <- read.table("list_Jun21_sel_AM_VOL_inp_files")
flist_data[1:4,]

ndata_file <- nrow(flist_data)

# read in the list containing the names of the stats files
flist_stats <- read.table("list_Jun21_sel_AM_VOL_stats_files")
...
#
write.csv(twopassoutsum,"Jun21_AM_VOL_twopassoutsum.csv", row.names=F)

write.csv(allkeysqt4rpass1,"Jun21_AM_VOL_allkeysqt4rpass1.csv", row.names=F)

if(length(allkeysqt4rpass2) > 0){
   write.csv(allkeysqt4rpass2,"Jun21_AM_VOL_allkeysqt4rpass2.csv", row.names=F)
   allkeysqt4rpass1$pass <- 1
   allkeysqt4rpass2$pass <- 2
   allkeysqt4rpass12 <- rbind(allkeysqt4rpass1, allkeysqt4rpass2)
   write.csv(allkeysqt4rpass12,"Jun21_AM_VOL_allkeysqt4rpass12.csv", row.names=F)
   } else {
   # just pass 1 but named as 12
   allkeysqt4rpass1$pass <- 1
   write.csv(allkeysqt4rpass1,"Jun21_AM_VOL_allkeysqt4rpass12.csv", row.names=F)   
   }
...
write.table(pass12_hilo,"Jun21_AM_VOL_pass12_hilo.txt", row.names=F)
