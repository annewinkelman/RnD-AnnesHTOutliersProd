# input data:
# ls -1v Jul21_sel_PM_VOL/*inp > list_Jul21_sel_PM_VOL_inp_files
# ls -1v Jul21_sel_PM_VOL/*stats > list_Jul21_sel_PM_VOL_stats_files

work_ls_num1_run <- paste0("ls -1v Jul21_sel_PM_VOL/*inp > list_Jul21_sel_PM_VOL_inp_files")
work_ls_num2_run <- paste0("ls -1v Jul21_sel_PM_VOL/*stats > list_Jul21_sel_PM_VOL_stats_files")
...
flist_data <- read.table("list_Jul21_sel_PM_VOL_inp_files")
flist_data[1:4,]

ndata_file <- nrow(flist_data)

# read in the list containing the names of the stats files
flist_stats <- read.table("list_Jul21_sel_PM_VOL_stats_files")
...
#for (ifile in 40:40){
     ...
     for (icgrp in 1:length(work_data_OUT)){
          print(paste("filename",work_filename,"CG",icgrp))
...
#	  work_age_grp_cd <- work_data_sel$AGE_GRP_CD
	  ...
	  sel_cat <- work_info$PM_VOL > 1.5 & work_info$abnormal == 0
	  ...
	  work_med <- median(work_info$PM_VOL)
	  work_mad <- mad(work_info$PM_VOL)

	  work_info$absvalue <- abs(work_info$PM_VOL - work_med)
	  work_info$maddev <- work_info$absvalue/work_mad
     
          work_info$outlier4 <- ifelse(work_info$maddev > work_qt4,1,0)
     	  # first pass
	  ipass <- 1
	  minnotout <- aggregate(work_info$PM_VOL, list(work_info$outlier4), min)
	  maxnotout <- aggregate(work_info$PM_VOL, list(work_info$outlier4), max)
	  # Side-by-side allowances: expand the bounds
	  # old: max((minnotout[1,2] - 1),1.5)
	  mincut <- max((minnotout[1,2] - 2),1.5)
	  maxcut <- maxnotout[1,2] + 2
	  pass1out4 <- work_info$PM_VOL < mincut | work_info$PM_VOL > maxcut
	  ...
	  workkey1 <- as.data.frame(work_info[outpos,c("file","anmlkey","Contemporary.Group","PM_VOL")])
	  workkey1$hilo <- ifelse(work_info[outpos,"PM_VOL"] < work_med,"low","high")
	  workkey1$ndata <- ndata1
	  allkeysqt4rpass1 <- rbind(allkeysqt4rpass1,workkey1)
	  }	
	  # big vs small
	  nsmall4r <- sum(work_info[outpos,"PM_VOL"] < work_med)
	  nbig4r <- sum(work_info[outpos,"PM_VOL"] > work_med)
	  # summary: start of CG
	  ...
	  ndata2 <- length(work_data2$PM_VOL)
	  meandata2 <- mean(work_data2$PM_VOL)
	  # medians
	  work_data2_med <- median(work_data2$PM_VOL)
	  # MAD default: mult by 1.4826
	  work_data2_mad <- mad(work_data2$PM_VOL)

	  # Calculate MAD: median(abs(x-6))
	  work_data2$absvalue <- abs(work_data2$PM_VOL - work_data2_med)
	  ...
	  minnotout <- aggregate(work_data2$PM_VOL, list(work_data2$outlier4), min)
	  maxnotout <- aggregate(work_data2$PM_VOL, list(work_data2$outlier4), max)

	  mincut <- max((minnotout[1,2] - 2),1.5)
	  maxcut <- maxnotout[1,2] + 2
	  pass2out4 <- work_data2$PM_VOL < mincut | work_data2$PM_VOL > maxcut
	  ...
	  # keys out with the second pass
     	  if (sum(work_data2$outlier4r==1) > 0){
              workkey2 <- as.data.frame(work_data2[outpos,c("file","anmlkey","Contemporary.Group","PM_VOL")])
	      workkey2$hilo <- ifelse(work_data2[outpos,"PM_VOL"] < work_data2_med,"low","high")
   	      workkey2$ndata <- ndata1 # should be 2?
   	      allkeysqt4rpass2 <- rbind(allkeysqt4rpass2,workkey2)
	 }
	 # big vs small
       	 nsmall <- sum(work_data2[outpos,"PM_VOL"] < work_data2_med)
       	 nbig <- sum(work_data2[outpos,"PM_VOL"] > work_data2_med)
	 # summary
...
#
write.csv(twopassoutsum,"Jul21_PM_VOL_twopassoutsum.csv", row.names=F)

write.csv(allkeysqt4rpass1,"Jul21_PM_VOL_allkeysqt4rpass1.csv", row.names=F)

if(length(allkeysqt4rpass2) > 0){
   write.csv(allkeysqt4rpass2,"Jul21_PM_VOL_allkeysqt4rpass2.csv", row.names=F)
   allkeysqt4rpass1$pass <- 1
   allkeysqt4rpass2$pass <- 2
   allkeysqt4rpass12 <- rbind(allkeysqt4rpass1, allkeysqt4rpass2)
   write.csv(allkeysqt4rpass12,"Jul21_PM_VOL_allkeysqt4rpass12.csv", row.names=F)
   } else {
   # just pass 1 but named as 12
   allkeysqt4rpass1$pass <- 1
   write.csv(allkeysqt4rpass1,"Jul21_PM_VOL_allkeysqt4rpass12.csv", row.names=F)   
   }
...
write.table(pass12_hilo,"Jul21_PM_VOL_pass12_hilo.txt", row.names=F)
...
write.table(npass1and2_df,"Jul21_PM_VOL_outlier_cnt.txt", row.names=F)
