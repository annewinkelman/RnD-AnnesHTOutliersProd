...
# files
two_pass_data <- read.csv("Nov21_PM_VOL_twopassoutsum.csv")

two_pass_data[1:4,]

# pass1 keys
# vars: "file","anmlkey","Contemporary.Group","PM_VOL","hilo","ndata"
keys_pass1 <- read.csv("Nov21_PM_VOL_allkeysqt4rpass1.csv")
keys_pass1[1:4,]
...
# vars: "file","anmlkey","Contemporary.Group","PM_VOL","hilo","ndata"
if(file.exists("Nov21_PM_VOL_allkeysqt4rpass2.csv")){
keys_pass2 <- read.csv("Nov21_PM_VOL_allkeysqt4rpass2.csv")
keys_pass2[1:4,]
keys_pass2$pass <- 2
}
...
#for (ifile in 1:1) {
     ...
     for (icgrp in 1:length(work_data_OUT)){
	  ...
	  if(sum(work_file_cat) > 0){
	     ...
	     sel_cat <- work_data_cgrp$PM_VOL > 1.5 & work_data_cgrp$abnormal == 0
	     ...
	     work_m1 <- merge(work_data_cgrp, work_sel_keys[,c("anmlkey","Contemporary.Group","PM_VOL","hilo","pass")], all.x=T, all.y=T)
	     ...
	     # ggplot     
	     work_plot <- ggplot(work_m1, aes(x=PM_VOL,fill=factor(pass))) +
