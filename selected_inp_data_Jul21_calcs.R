...
sel_file_names_list <- read.table("list_Jul21_status1_files_names")
...
sel_stats_file_list <- read.table("selected_Jul21_outlier_stats_files")
...
sel_cnts_file_list <- read.table("selected_Jul21_outlier_cnts_files")
...
sel_data_file_list <- read.table("selected_Jul21_input_files")
...
#for (ifile in 182:182) {
	...
	if(!work_file_size){
	   work_trait_name <- trait_list[itrait]
       	   work_dir <- paste0("Jul21_sel_",work_trait_name,"/")
       	   # cnts
