...
names(cgdata4)[c(2,15)] <- c("Contemporary.Group","actual_pm_vol")
...
names(cg_max) <- c("Contemporary.Group","max")
...
for(ic in 1:length(OUT4)){
    ...
    work_mean <- mean(work_data$actual_pm_vol)
    work_sd <- sd(work_data$actual_pm_vol)    
# mad calc stats
    ...
    # *** plot ***
    work_col <- ifelse(work_data$outlier4 == 1, "red", "green")
    hist(work_data$actual_pm_vol, main=paste0("Histogram of AM Volume in CG ",work_cg,"\n", "N HTs: ", work_n ," N Outliers: ", n_total_outlier4), xlab="Litres", col="lightblue")
    ...
    # cg id
    work_cg_id <- c(work_cg, work_n, work_mean, work_med, work_sd, n_total_outlier4, nsmall, nbig)
...
names(all_cgs_stats_df) <- c("Contemporary.Group","N","Mean", "Median", "SD", "outlier4", "nsmall","nbig")
