...
# "carryover","dim","actual_prot_pct","milk_abnm_cd","species_descr","Nstart","cg","qt4","mad","med","absvalue","maddev","outlier4","small","big"
cgdata4 <- read.csv("prot_pct_recalc_size1_data.csv")
nrow(cgdata4)
...
for(ic in 1:2){
   ...
   ndata <- length(workdata$actual_prot_pct)
# pass 1
   ndata1 <- ndata
# The original outlier4 is used here
   ipass <- 1
   minnotout <- aggregate(workdata$actual_prot_pct, list(workdata$outlier4), min)
   maxnotout <- aggregate(workdata$actual_prot_pct, list(workdata$outlier4), max)
# Side-by-side allowances: expand the bounds
# have to have at least 0.5%
   mincut <- max((minnotout[1,2] - 0.25),0.5)
   maxcut <- maxnotout[1,2] + 0.25
# so what wasn't a low outlier under basic qt4 could become one with the new mincut.  Is that valid?
   pass1out4 <- workdata$actual_prot_pct < mincut | workdata$actual_prot_pct > maxcut
   workdata$outlier4r <- ifelse(pass1out4,1,0)
   workmed <- median(workdata$actual_prot_pct)
   outpos <- which(workdata$outlier4r==1) # relaxed
# keys
# These are out with the first pass
   if (sum(workdata$outlier4r==1) > 0){
   workkey1 <- as.data.frame(workdata[outpos,c("anml_key","Contemporary.Group","actual_prot_pct","qt4","maddev")])
   workkey1$mincut <- mincut
   workkey1$maxcut <- maxcut   
   workkey1$hilo <- ifelse(workdata[outpos,"actual_prot_pct"] < workmed,"low","high")
   ...
   if (sum(workdata$outlier4==1 & workdata$outlier4r==0) > 0){
      workkey1 <- as.data.frame(workdata[outpos_reprieve,c("anml_key","Contemporary.Group","actual_prot_pct","qt4","maddev")])
      workkey1$mincut <- mincut
      workkey1$maxcut <- maxcut
...
# big vs small
   nsmall4r <- sum(workdata[outpos,"actual_prot_pct"] < workmed)
   nbig4r <- sum(workdata[outpos,"actual_prot_pct"] > workmed)
...
# First do q4
   ...
   meandata <- mean(workdata$actual_prot_pct)
# stats
   stddata <- sd(workdata$actual_prot_pct)
   devdata <- workdata$actual_prot_pct - meandata
...
# RECALCS for reduced data - done CG by CG here
       ...
       meandata <- mean(workdata$actual_prot_pct)
# medians
       workdata$med <- median(workdata$actual_prot_pct)
# mad default: mult by 1.4826
       workdata$mad <- mad(workdata$actual_prot_pct)

       # Calculate mad: median(abs(x-6))
       workdata$absvalue <- abs(workdata$actual_prot_pct - workdata$med)
...
# 29 May 2018: Here we are working with outlier4!
       ...
       mincut <- max((minnotout[1,2] - 0.25),0.5)
       maxcut <- maxnotout[1,2] + 0.25
       pass2out4 <- workdata$actual_prot_pct < mincut | workdata$actual_prot_pct > maxcut
...
# keys out with the second pass
       if (sum(workdata$outlier4r==1) > 0){
          workkey2 <- as.data.frame(workdata[outpos,c("anml_key","Contemporary.Group","actual_prot_pct")])
	  workkey2$hilo <- ifelse(workdata[outpos,"actual_prot_pct"] < workmed,"low","high")
...
# keys that got a reprieve in the second pass
   outpos_reprieve <- which(workdata$outlier4==1 & workdata$outlier4r==0)
   if (sum(workdata$outlier4==1 & workdata$outlier4r==0) > 0){
      workkey2 <- as.data.frame(workdata[outpos_reprieve,c("anml_key","Contemporary.Group","actual_prot_pct","qt4","maddev")])
...
# big vs small
       nsmall <- sum(workdata[outpos,"actual_prot_pct"] < workmed)
       nbig <- sum(workdata[outpos,"actual_prot_pct"] > workmed)
# summary
# Just too see:      outsum1 <- c(cgnum, cggage,ndata, workdata$mad[1], skewdata, kurtosisdata, sum(workdata$outlier4), sum(workdata$outlier4r),  nbig4r, nsmall4r)
       ...
       work_info <- cbind(workdata$anml_key, workdata$actual_prot_pct, workdata$outlier4, workdata$outlier4r, mincut, maxcut)
...
write.csv(twopassoutsum,"protpct_size1_all2passum.csv", row.names=F)
...
# keys
# pass 1
write.csv(allkeysqt4rpass1,"protpct_size1_allkeysqt4rpass1.csv", row.names=F, quote=F)

# pass2
write.csv(allkeysqt4rpass2,"protpct_size1_allkeysqt4rpass2.csv", row.names=F, quote=F)


# data sets
all_in_selcols <- all_in_htdata[,c("Contemporary.Group", "anml_key","actual_prot_pct","Nstart","outlier4","outlier4r")]

all_fin_selcols <- all_fin_htdata[,c("Contemporary.Group", "anml_key","actual_prot_pct","outlier4","outlier4r")]
...
all_fin_selcols <- all_fin_htdata[,c("Contemporary.Group", "anml_key","actual_prot_pct","Nstart","outlier4","outlier4r")]
...
#
write.csv(final_data,"protpct_size1_level4_outlier_id.csv", row.names=F)
