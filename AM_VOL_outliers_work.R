...
#	  work_age_grp_cd <- work_data_sel$AGE_GRP_CD
	  ...
	  # data storage
	  work_store1 <- work_info[,c("anmlkey","AM_VOL","outlier4","outlier4r")]
	 ...
	 # summary
 	 ...
 	 work_store2 <- work_data2[,c("anmlkey","outlier4","outlier4r")]
	 names(work_store2) <- c("anmlkey","outlier4_2","outlier4r_2")
...
all_store <- merge(work_store1, work_store2, all.x=T, ally=T)

all_store[is.na(all_store)] <- 0

all_store[1:4,]


aorder <- order(-all_store$AM_VOL)

all_store_o <- all_store[aorder,]


all_store_o[1:10,]

cat_ge10 <- work_info$AM_VOL >= 10.0

nrow(work_info)
work_info2 <- work_info[!cat_ge10,]
nrow(work_info2)
