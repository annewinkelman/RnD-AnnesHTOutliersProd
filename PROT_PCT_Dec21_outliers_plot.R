...
# files
two_pass_data <- read.csv("Dec21_PROT_PCT_twopassoutsum.csv")

two_pass_data[1:4,]

# pass1 keys
# vars: "file","anmlkey","Contemporary.Group","PROT_PCT","hilo","ndata"
keys_pass1 <- read.csv("Dec21_PROT_PCT_allkeysqt4rpass1.csv")
keys_pass1[1:4,]
...
# vars: "file","anmlkey","Contemporary.Group","PROT_PCT","hilo","ndata"
if(file.exists("Dec21_PROT_PCT_allkeysqt4rpass2.csv")){
keys_pass2 <- read.csv("Dec21_PROT_PCT_allkeysqt4rpass2.csv")
keys_pass2[1:4,]
keys_pass2$pass <- 2
}
...
pdf("Dec21_PROT_PCT_outliers.pdf")
...
#for (ifile in 1:1) {
     ...
     for (icgrp in 1:length(work_data_OUT)){
	  ...
	  if(sum(work_file_cat) > 0){
	     # still need to remove records below the minimum cut-off
	     sel_cat <- work_data_cgrp$PROT_PCT > 0.5 & work_data_cgrp$abnormal == 0
